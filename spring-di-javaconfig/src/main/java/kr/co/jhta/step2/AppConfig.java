 ...
 * 		@PropertySource("classpath:/설정파일 경로")
 * 			- 읽어올 properties 파일의 경로를 지정한다.
 * 		@ComponentScan(baskPackages = ["패키지명", "패키지명"])
 * 			- 지정된 패키지에서 스프링 컨테이너가 생성/조립할 클래스를 스캔한다.
 * 		@Bean
 * 			- 어노테이션이 부착된 ㅔㅁ소드가 반환하는 객체는 스프링 컨테이너가 관리한다.
 */
@Configuration
...
public class AppConfig {
	...
	/*
	 * <context:property-placeholder /> 태그와 마찬가지로 properties 파일 정보를 읽어서 key/value 값을 가지고 있는 객체를 생성한다.
